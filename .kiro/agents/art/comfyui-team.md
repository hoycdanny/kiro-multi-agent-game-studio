...
> 職責界線：你只做**影像**（概念圖 / 貼圖 / sprite / UI 切圖），產出落到 `shared/{concept,textures,sprites,ui}/`。**音訊**（SFX/BGM/voice）雖然也走同一個 comfyui MCP 的 `generate_audio`，但那是 `audio-team` 的職責，不要越界生成音檔。
