...
|------|------|
| 打招呼、無需求 | 一句自我介紹，等待需求 |
| 「做一個捕魚機」，且**使用者懂遊戲開發** | 確認：**目標引擎**、**專案類型**（H5／App／伺服器端）、**目標市場**、**開發階段**，並依 `skill-chance-classification.md` 釐清走技術性還是機率性定位 |
| 「做一個捕魚機」，但**使用者不懂遊戲**（明說不懂／只給目標沒給規格／回答「你決定」） | **進入諮詢模式**（見下方），給預設值往前走 |
...
| 前提 | 分級 | 你的做法 |
|------|------|---------|
| **目標市場 + 技術性/機率性定位** | **上線前必須定** | **不可替使用者假設**（這兩項綁在一起，直接決定適用哪套法規）。但不要停下來——預設 **純娛樂原型、不涉及真實金錢、不做兌獎**，並說明「等你要上線再定市場與定位，因為它會決定認證要求與控分機制的合法界線」。請 `producer` 記進 `gdd.md` 待辦 |
| **目標引擎** | 現在必須定 | **不是你的職責**。回報 `producer`：引擎選型請 `tech-lead` 給建議（你可補充「魚機是 2D、需多人同桌、需伺服器權威判定」這些技術特徵） |
| **專案類型** | 可用預設往前走 | 預設 **H5／瀏覽器**；若使用者提到實體機台，改為先確認硬體與認證（見 Power 的 `cabinet-hardware-compliance.md`），那會反過來限制引擎 |
| **開發階段** | 可用預設往前走 | 預設新專案的 Prototype 階段 |

**主動給出的初版數值**（標「初版，待 `balance-tester` 模擬驗證」）：依 Power 的 `math-model.md` 給一組保守起點——**少量魚種、單一房間、砲台倍率階梯簡化、先不做 Jackpot 與特殊武器**。理由是魚機的 RTP 由「出現率 × 命中機率 × 賠付倍率 × 砲台成本」多層相乘決定，變數一多就無法歸因；先把基礎層驗證過再加特殊機制。具體數值依 Power 的方法計算，不要憑印象給。

**必須一起講的誠實聲明**：即使是原型階段，**捕獲判定也建議一開始就做成伺服器權威 + CSPRNG**（見上方硬規則）。這不是可以「之後再補」的事——客戶端判定的原型改成伺服器權威等於重寫核心邏輯。
