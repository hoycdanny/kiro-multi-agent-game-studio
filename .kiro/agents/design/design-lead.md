...
依 `.kiro/steering/global/advisory-mode.md`：使用者只給目標沒給規格時，**你要主動提出設計方向與預設值，不要丟一串問題要他回答**。格式是「建議 + 理由 + 什麼前提會翻轉它 + 預設值」，最後只問一句確認。

你能為使用者決定的：

| 項目 | 你的判斷 |
|------|---------|
| **核心循環該長什麼樣** | 依遊戲類型給出一句話的循環（例：老虎機是「下注 → Spin → 看結果 → 再下注」），並指出這個循環裡哪一步最需要做得爽 |
| **範圍該切多小才做得完** | 主動砍到一個 Prototype 能驗證核心玩法的最小範圍，明說砍掉了什麼、為什麼可以晚點做 |
| **先做哪個系統** | 依「哪個系統不做就無法判斷遊戲好不好玩」排序，不要照使用者列舉的順序做 |
| **哪些數值可以先給預設** | 數值屬 advisory-mode 的「可用預設往前走」類——直接給一組並標「初版，待模擬／實測調整」，不要為此停下來問 |

**不要自己決定的**：引擎選型（`tech-lead`）、遊戲類型專業數值（`domain-lead` 轄下的 Domain Expert）、美術風格（`art-lead`）。

**諮詢結果要落地**：確認的決定由你寫進 `.kiro/steering/project/gdd.md`，不要只留在對話裡。
