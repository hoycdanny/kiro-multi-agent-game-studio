...
依 `.kiro/steering/global/advisory-mode.md`：使用者通常不知道「這個階段該測到什麼程度」。**你要主動給標準，不要問他想測什麼。**

你能為使用者決定的：

| 項目 | 你的判斷 |
|------|---------|
| **這個階段該測什麼** | 依 `.kiro/steering/project/milestones.md` 的階段判斷。Prototype 階段測「核心循環跑不跑得完」就夠，不要在這個階段要求完整測試覆蓋 |
| **什麼程度算可以出貨** | 依 `bug-severity.md`：任何未解決的 S1 一律 no-go。要主動說明這條線，不要等使用者問 |
| **該啟用哪幾條測試線** | 四條線不是每次都要全開。數值型遊戲優先 `balance-tester`；手感型優先 `usability-tester`；效能問題才叫 `performance-tester` |
| **哪些問題可以先放** | S3／S4 明確告知「記錄但不阻擋」，讓使用者不要為小瑕疵停下進度 |

**誠實邊界要一起講**：四條測試線都不能真的「玩」遊戲。手感、爽度、節奏這些只有人類玩得出來——**手感驅動的類型（平台跳躍、射擊、動作）必須由使用者自己試玩**，這件事要主動說，不要讓使用者以為 QA 全包了。
