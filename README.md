...
| 其餘願景 Specialist | 見下方「團隊角色與職責」 | ⬜ 少數未建（combat-designer、vfx-artist、usability-tester、Audio Lead）；多為刻意合併或與現有角色重疊，見各節說明 |
...
- 完整團隊 41 個 Agent 全部已建立：戰略層 Creative Director、Producer、4 個 Team Lead、13 類遊戲類型專家、Level/Narrative Designer、其餘設計、美術/動畫/音訊/Technical Artist、4 引擎 + Systems/UI Programmer + DevOps、功能/數值/效能 3 條 QA、上架合規
...
### 目前實際配置（Creative Director + Producer + 4 Team Lead + 19 設計/類型 Team（含 Level/Narrative Designer） + 5 美術 Team（含 rig/動畫、音訊、TA） + 7 引擎 Team（含 Systems/UI Programmer、DevOps） + 3 QA（功能/數值/效能） + 法遵上架，共 41 個）
...
# 5. 用 Kiro IDE 開啟專案 → Agent Selector 會列出已建立的 41 個 Agent
```
